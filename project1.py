...
def average_perceptron(feature_matrix, labels, T):
    ...
    """
    theta = theta_sum = np.zeros(feature_matrix.shape[1])
    theta_0 =  theta_0_sum = 0.0
...
def pegasos_single_step_update(
    ...
    if z<= 1:
        updated_theta += eta*label * feature_vector
        updated_theta_0 += eta*label
    return updated_theta, updated_theta_0
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range(n):
        evaluation = np.dot(theta, feature_matrix[i])+theta_0
        predictions[i] = 1 if evaluation > 0 else -1
    return predictions
...
def bag_of_words(texts, stopwords, remove_stopword=True):
    """
    ...
    Args:
        ...
        `stopwords` - a list of natural language strings we wish to ignore.
    Returns:
